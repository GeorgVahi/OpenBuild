...
OpenBuild accepts focused changes against `main`. Keep the public plugin self-contained, preserve English/Russian documentation parity, and do not add personal model IDs, local paths, secrets, or dependencies on private skills. The reviewed packaged `gpt-5.3-codex-spark` discovery profile is the only intentional fixed-model exception.
...
For search-dispatch changes, the deterministic trace fixture must prove that exact `agent_name=openbuild_search_separate` selection, a separate descriptive `task_name`, and a complete routing receipt precede the first repository-search event. It must also reject root-first search, `task_name` used as a selector, generic dispatch without a recorded exact-selector failure, and unknown fallback reasons. A maintainer runtime smoke runs the packaged `scripts/agent_runner.py` in a fresh context with a small repository-mapping prompt; pass only when the receipt records `codex-exec-explicit-model`, configured and observed `gpt-5.3-codex-spark`, low reasoning, read-only sandbox, Codex PID/thread ID, and terminal `turn.completed` before root verification. Treat the usage dashboard as optional secondary evidence.
