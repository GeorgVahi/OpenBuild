...
Current release: `2.4.0-alpha.3` ([pinned skill source](https://github.com/GeorgVahi/OpenBuild/tree/v2.4.0-alpha.3/plugins/openbuild/skills/build)).
...
Version 2.4.0-alpha.3 adds the M3a project-wide scope/resource reservation core above the existing per-lane writer lifecycle. File and directory ancestor aliases, contract keys, and exclusive resource keys now converge through generation-CAS reservations; soft intents remain scheduler hints without write authority. Simultaneous conflicting admissions deterministically produce one active owner and one durable `waiting-for-scope` lane, with monotonic ticket order preserved. Pre-start expansion is all-or-wait, post-write input is rejected, and inactive dynamic cycles cancel only the newer reservation that forms the cycle edge. Runner activation rechecks the active hard grant. Live-writer expansion, live-cycle safe-stop and scope release remain deliberately fail-closed until the R-032 runner/integration owner handshakes land in the next prerelease; DAG scheduling, integration and the stable 2.4.0 release remain later milestones.
