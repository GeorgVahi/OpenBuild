...
Current release: `2.2.2` ([pinned skill source](https://github.com/GeorgVahi/OpenBuild/tree/v2.2.2/plugins/openbuild/skills/build)).
...
Version 2.2.2 stages every task prompt in owner-private storage before dispatch; compatibility prompt files must be external, stable, owner-controlled, and privately permissioned. OpenBuild writes no workspace path while an implementation lease is retained. When a stopped transport-success lifecycle has full-tree-zero evidence and exact `outside-set-drift`, it automatically records a no-handoff terminal abandonment, invalidates the checkpoint, closes containment, and releases that same lease. It then uses existing root-completion authority only for an attributable same-revision/same-scope diff. Genuine user decisions remain `decision-required`; ambiguous safety evidence is `blocked`; exhausted safe automation is `automation-exhausted`. None of those states silently starts a recovery writer or asks for permission that cannot alter the evidence.
