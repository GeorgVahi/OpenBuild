...
Current release: `2.4.0-alpha.4` ([pinned skill source](https://github.com/GeorgVahi/OpenBuild/tree/v2.4.0-alpha.4/plugins/openbuild/skills/build)).
...
```powershell
codex plugin marketplace add GeorgVahi/OpenBuild --ref v2.4.0-alpha.4
...
Version 2.4.0-alpha.4 completes the M3 project-wide scope/resource lifecycle. A running lane now consumes a generation-bound safe-stop intent, proves its exact process tree is empty, and either rebinds a clean expanded allowed set or preserves dirty work as a checkpoint-bound `recovery-ready` lane. Safe-stop completion is persisted in the project lane before the runner-local receipt, so a crash at that boundary replays without closing the rebound lane. Every writer attach must match the exact active lease in that lane's session-bound recovery registry; generic detach/reattach CAS substitutions are rejected. A recovery target may continue only across its already-active scopes while a conflicting expansion remains waiting. The lane-local recovery root is immutable session state, so direct acceptance cannot substitute another registry. Scope release requires its exact successful terminal archive, including the bound writer `run_id`, a non-empty committed lane result at the integration-ref tip, and a validation command executed by the durable owner in a separate owner-private detached checkout of that accepted commit, with clean before/after HEAD, tree and status evidence; caller-generated digests, empty commits, changed writer/worktree identities, changed validation checkouts and cross-lane release attempts are rejected. The real two-lane fixture completes that dirty recovery, integrates and releases the coherent prefix, cancels the unmet expansion, and then proves the neighboring live lane progresses to terminal. DAG scheduling, the full integration queue and the stable 2.4.0 release remain later milestones.
