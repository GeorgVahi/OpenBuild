...
Preflight Python 3.11 or newer (`python` on Windows, commonly `python3` on POSIX), resolve `<build-skill-root>` from this selected skill's on-disk path, write one bounded UTF-8 prompt file per task, and start the selected role without shell interpolation:
