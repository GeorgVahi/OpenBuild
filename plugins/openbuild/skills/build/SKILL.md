...
Build an ordered ladder only from native per-spawn selectors or confirmed custom agent profiles. Dispatch the exact starting reviewer from risk: `openbuild-review-fast` for low, `openbuild-review-balanced` for medium, `openbuild-review-strong` for high, and `openbuild-review-strongest` for critical. Bind a confirmed model directly when the spawn schema exposes a selector; otherwise select the exact custom-agent name. Emit a Review routing receipt before consuming every result. A discovery tier may be cheaper than the required final review tier.
