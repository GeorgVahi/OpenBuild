...
After stopped transport success and authenticated full-tree zero, exact `[outside-set-drift]` with no handoff/outbox is reconciled automatically through private `_reconcile-terminal-abandonment`. The exact owner-derived `terminal-abandonment-v1` record changes terminal success to false, permanently invalidates the source checkpoint, produces no retry/escalation/grant/new run/new writer, then uses the existing guardian-close/archive/release gates. Caller cause/digest input, mixed reasons, live or unknown process state, containment/identity/ownership ambiguity, and quarantine are fail-closed without mutation or force-unlock.
