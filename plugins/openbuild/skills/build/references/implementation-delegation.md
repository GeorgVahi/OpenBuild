...
After stopped transport success and authenticated full-tree zero, exact `[outside-set-drift]` with no handoff/outbox is reconciled automatically through private `_reconcile-terminal-abandonment`. The exact owner-derived `terminal-abandonment-v1` record changes terminal success to false, permanently invalidates the source checkpoint, produces no retry/escalation/grant/new run/new writer, then uses the existing guardian-close/archive/release gates. Only a recovery-target may use the exact sorted pair `[outside-set-drift, preexisting-dirty-overlap]`; the same private command records owner-derived `terminal-abandonment-v2`, uses its distinct invalidation reason, retires the consumed recovery authorization, and reaches vacancy through the existing gates without accepting a handoff or creating root-completion authority. Root completion remains a separate post-vacancy audit. Caller cause/digest input, the same pair on a normal lease, any additional/unknown/control-plane reason, live or unknown process state, containment/identity/ownership ambiguity, and quarantine are fail-closed without mutation or force-unlock.
