...
- recovery-autonomy diffs prove owner-private prompt staging, no root workspace write during a non-vacant implementation lease, exact same-lifecycle abandonment with no handoff/new writer, and the closed decision/blocker/exhaustion boundary without a useless permission prompt; recovery-overlap diffs additionally prove recovery-target-only exact `[outside-set-drift, preexisting-dirty-overlap]`, v2 schema/binding/fault replay, normal/additional-reason no-mutation, 2.2.0–2.2.3 reader-floor/legacy replay with pending-v2 downgrade rejection, and separate post-vacancy root audit without diff acceptance or Git/workspace mutation;
