...
4. Select the risk-matched root or bounded implementation worker, acquire the single-writer lease for that exact profile, then `start` it with the lease ID as required by [adaptive implementation delegation](implementation-delegation.md). Record the unactivated `running` Implementation routing receipt, call `activate`, and record the matching `implementation-agent-activated` event before any test or production code edit; start `openbuild_implementation_fast` for low, `openbuild_implementation_balanced` for medium or high, and `openbuild_implementation_strongest` for critical. A completed worker may return `NEEDS_ESCALATION` before any edit; after the root verifies zero writes and terminal success, release the lease and advance exactly one tier within the risk ceiling. Infrastructure or transport failure never authorizes escalation. If the exact route is unavailable or fails, stop implementation, confirm that no worker process remains active, keep the milestone blocked, and create no replacement writer. Once any edit occurs, keep the same writer for the remaining red/green cycle.
...
9. Require the terminal Implementation routing receipt. A successful receipt permits the run-bound `implementation-handoff-accepted` event and result consumption before lease release; a matching failed/cancelled receipt with complete failure evidence and the process tree stopped forbids accepted handoff and permits lease release only while the milestone stays incomplete. Then complete the applicable root handoff or remediation gate, run wider validation according to risk, and update durable documentation when behavior, commands, or contracts changed.
