...
4. Select the risk-matched root or bounded implementation worker, acquire the single-writer lease for that exact profile, then `start` it with the lease ID as required by [adaptive implementation delegation](implementation-delegation.md). Record the unactivated `running` Implementation routing receipt, call `activate`, and record the matching `implementation-agent-activated` event before any test or production code edit; require `openbuild_implementation_fast` for low-risk Direct work, `openbuild_implementation_balanced` for medium contained behavior, and `openbuild_implementation_strongest` for high or critical work. If the required exact tier is unavailable or fails, stop implementation, confirm that no worker process remains active, keep the milestone blocked, and create no replacement writer.
