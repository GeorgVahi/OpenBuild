...
4. Resolve `implementation.<risk>` through the effective model map, select its first exact root or bounded implementation worker, acquire the single-writer lease for that profile, then `start` it with the lease ID as required by [adaptive implementation delegation](implementation-delegation.md). Record the map source/hash and route step, the unactivated `running` Implementation routing receipt, call `activate`, and record the matching `implementation-agent-activated` event before any test or production code edit. A completed worker may return a configured `NEEDS_ESCALATION` trigger before any edit; after the root verifies zero writes and terminal success, release the lease and advance exactly one configured route step. Infrastructure or transport failure never authorizes escalation. If the exact route is unavailable or fails, stop implementation, confirm that no worker process remains active, keep the milestone blocked, and create no replacement writer. Once any edit occurs, keep the same writer for the remaining red/green cycle.
