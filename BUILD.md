...
| Version policy | `CONTRIBUTING.md:16-31`, `plugins/openbuild/.codex-plugin/plugin.json:3` | Manifest — authoritative source; breaking contract требует major и синхронизацию CHANGELOG/README. | Development surfaces синхронизированы на `2.0.0`; latest published immutable release остаётся `v1.1.1`. |
...
| Current package version | `plugins/openbuild/.codex-plugin/plugin.json:3`, `README.md` | Working tree использует development version `2.0.0`; latest published immutable release остаётся `v1.1.1`, tag/Release `v2.0.0` не создавались. | M3 major bump применён честно без ложного release claim; M4–M6 ещё не завершены. |
