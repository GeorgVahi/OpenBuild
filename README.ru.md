...
Текущий релиз: `2.4.0-alpha.3` ([закреплённый исходник skill](https://github.com/GeorgVahi/OpenBuild/tree/v2.4.0-alpha.3/plugins/openbuild/skills/build)).
...
```powershell
codex plugin marketplace add GeorgVahi/OpenBuild --ref v2.4.0-alpha.3
...
Версия 2.4.0-alpha.3 добавляет project-wide M3a core для scope/resource reservations поверх существующего lifecycle одного writer в каждой lane. Ancestor aliases для file/directory, contract keys и exclusive resource keys теперь сходятся через generation-CAS reservations; soft intents остаются подсказками scheduler без write authority. Две одновременные конфликтующие заявки детерминированно дают одного active owner и одну durable lane `waiting-for-scope`, сохраняя monotonic ticket order. Pre-start expansion работает целиком или переходит в ожидание, post-write input отклоняется, а inactive dynamic cycles отменяют только более новую reservation, которая образует cycle edge. Runner activation повторно проверяет active hard grant. Live-writer expansion, live-cycle safe-stop и scope release намеренно остаются fail-closed до следующего prerelease с R-032 runner/integration owner handshakes; DAG scheduling, integration и стабильный релиз 2.4.0 остаются следующими milestones.
