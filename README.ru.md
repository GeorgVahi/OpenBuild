...
Текущий релиз: `2.3.1` ([закреплённый исходник skill](https://github.com/GeorgVahi/OpenBuild/tree/v2.3.1/plugins/openbuild/skills/build)).
...
```powershell
codex plugin marketplace add GeorgVahi/OpenBuild --ref v2.3.1
...
Версия 2.3.1 дополнительно требует корректный ненулевой creation-bound Codex exit до любой авторизации Terra по структурированному Spark availability failure; нулевой exit остаётся fail-closed даже при противоречивом failed-run envelope. Версия 2.3.0 закрывает описанное recovery-зависание без принятия неоднозначного diff. После успешной остановки transport и подтверждённого full-tree zero только recovery target с точными отсортированными reasons `[outside-set-drift, preexisting-dirty-overlap]` может использовать owner-derived `terminal-abandonment-v2`. Переход навсегда инвалидирует checkpoint отдельной причиной, завершает использованную recovery-авторизацию и повторно использует существующие identity, guardian, archive и release gates. Он не создаёт handoff, retry, escalation, grant, writer или root-completion authority; та же пара на normal lease и любые дополнительные, неизвестные или control-plane reasons остаются fail-closed без мутации или force-unlock. Root completion выполняется отдельным post-vacancy audit. Первый durable write нового owner поднимает reader floor до 2.2.5; точные floors 2.2.0–2.2.3 читаются без rewrite, а старый reader не может открыть non-vacant registry с floor 2.2.5 до явного vacant retirement.
