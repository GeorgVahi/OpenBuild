...
Текущий релиз: `2.4.0-alpha.4` ([закреплённый исходник skill](https://github.com/GeorgVahi/OpenBuild/tree/v2.4.0-alpha.4/plugins/openbuild/skills/build)).
...
Версия 2.4.0-alpha.4 завершает project-wide lifecycle scope/resource для M3. Работающая lane теперь потребляет generation-bound safe-stop intent, доказывает опустошение своего точного process tree и либо получает чистый rebind расширенного allowed set, либо сохраняет dirty-работу как checkpoint-bound `recovery-ready`. Safe-stop completion сохраняется в project lane до runner-local receipt, поэтому crash на этой границе replay-ится без закрытия rebound lane. Каждый writer attach обязан совпадать с точным active lease в session-bound recovery registry этой lane; generic detach/reattach CAS-подмена отклоняется. Recovery target может продолжать только по уже active scopes, пока конфликтующее expansion остаётся waiting. Lane-local recovery root становится неизменяемой частью project session, поэтому direct acceptance не может подставить другую registry. Scope release требует точный успешный terminal archive с привязанным writer `run_id`, непустой committed result lane на tip integration ref и validation-команду, которую durable owner выполняет в отдельном owner-private detached checkout этого accepted commit с чистыми before/after evidence HEAD, tree и status; caller-generated digest, empty commit, подмена writer/worktree identity, изменившийся validation checkout и попытка release чужой lane отклоняются. Реальный двух-lane fixture завершает dirty recovery, интегрирует и освобождает coherent prefix, отменяет невыполнимое expansion и затем доказывает, что соседняя live lane доходит до terminal state. DAG scheduler, полная integration queue и стабильный релиз 2.4.0 остаются следующими milestones.
