...
Текущий релиз: `2.2.2` ([закреплённый исходник skill](https://github.com/GeorgVahi/OpenBuild/tree/v2.2.2/plugins/openbuild/skills/build)).
...
```powershell
codex plugin marketplace add GeorgVahi/OpenBuild --ref v2.2.2
...
В версии 2.2.2 каждый task prompt до dispatch сохраняется в owner-private storage; compatibility prompt-файл обязан находиться вне workspace, быть стабильно открываемым, принадлежать текущему пользователю и иметь приватные права. Пока implementation lease удерживается, OpenBuild не пишет ни в один workspace path. Если остановленный transport-success lifecycle имеет full-tree-zero evidence и exact `outside-set-drift`, OpenBuild автоматически фиксирует terminal abandonment без handoff, инвалидирует checkpoint, закрывает containment и освобождает тот же lease. После этого существующее root-completion authority применяется только к атрибутируемому diff той же revision и scope. Настоящий пользовательский выбор возвращает `decision-required`, неоднозначное safety evidence — `blocked`, а исчерпание безопасной автоматизации — `automation-exhausted`. Ни один из этих исходов скрыто не запускает recovery writer и не просит разрешение, которое не может изменить evidence.
