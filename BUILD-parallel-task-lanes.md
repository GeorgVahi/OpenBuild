...
- Status: In progress — M1 and M2 complete in the validated `2.4.0-alpha.2` release candidate; M3 scope/resource scheduling is next
...
|---|---|---|---|---|---|---|
| `BUILD-parallel-task-lanes.md` | пользователь + root synthesis | R-031 | Новый capability, D-001–D-010, T-001–T-021, AC-01–AC-38 | I0 capability, permanent key, immutable anchor lock, automatic first-invocation setup and validator-safe transition IDs close bootstrap/release ownership | yes | root source |
...
- [ ] AC-17: Port/test DB/Docker collision test доказывает уникальный lane namespace либо serialization через resource lease.
...
- [ ] AC-22: Stress fixture с десятью lanes, смешанными scopes и bounded test capacity завершается без lost update, double-owner, starvation и global stop; порядок интеграции воспроизводим.
...
- Status: Pending
- Scope: canonical scopes, hard grants, soft intents, dynamic expansion, waiting, fairness, cycle handling.
- Excludes: Git integration content merge.
- Implementation mode: TDD-first
- Delegation: bounded-worker, high-risk route.
- Red signal: double-claim/alias/dynamic-cycle fixtures.
- Focused green: scope state-machine and multiprocessing race tests.
...
- Focused green: integration fault/replay/conflict/stale tests.
...
- Focused green: resource namespace/queue stress.
...
| B-023 | specification source-graph completeness | covered | repository fact | R-031 maps every normative feature/release/readiness companion, all direct Markdown edges, public interface/docs assets/legal/published/attribution links, and direct executable/test owners with explicit authority limits | fresh R-031 closure |
...
| Shared ports/DB создают flaky tests | high/high | namespace/resource leases и capacity scheduler | Open until M6 |
...
Created logical agent runs through the exact OpenBuild runner: `42`.
