...
- Status: In progress — M1–M7 complete; M7 root self-review override accepted after exact-review transport failure, M8 stable publication authorized and in progress
...
- [x] AC-19: Path escape, symlink/reparse ancestor, Git common-directory replacement и identity drift fail-closed без чужого scope release.
- [x] AC-20: Restart/replay после fault в I0 root/lock/key/capability, BA0 temp/directory publish/record/intent/handoff, reservation/activation/terminal/integration, clean B0 visibility, BS alias writes/CAS/clear/gen-0 completion и record compaction даёт one immutable anchor-lock identity и one authoritative outcome без double-owner, orphan clean BS, second epoch или lost fence.
- [x] AC-21: Exact legacy registry fixtures до 2.3.6 завершаются по прежней матрице; новая project registry имеет явный reader floor и безопасную migration/retirement policy.
...
- [x] AC-23: README EN/RU, SKILL/reference contracts, manifest/changelog и diagrams описывают одинаковую модель «parallel tasks, one writer per lane, one integrator».
- [x] AC-24: Полный unittest/package gate, включая T-021 positive registered-transition и negative fixed-model controls, и fresh high-risk progressive review проходят без actionable findings; при exact-review transport failure допустим только явно авторизованный пользователем root self-review override с полной root validation и без заявления independence; actual release/push выполняются только при отдельной authority.
...
- [x] AC-28: Writer-floor promotion и mixed-version fixtures отклоняют lower-version project transition без mutation; active discoverable legacy lane остаётся external/protected, не освобождает и не получает managed scope, а conflicting project admission ждёт её доказанной vacancy.
...
- [x] AC-35: Commit-gate fixture от `2.3.6` принимает последовательность `2.4.0-alpha.1 ... 2.4.0-alpha.N -> 2.4.0` с синхронными manifest/changelog/README в каждом non-empty commit и отклоняет duplicate, downgrade, ранний stable target, commit после stable на той же линии и mismatched documentation pins.
- [x] AC-36: Две lanes, готовые к integration одновременно, получают разные monotonic prerelease tickets только в фактическом integration order; injected reorder/failure после allocation не переиспользует ticket, stale commit не интегрируется как downgrade, version surfaces изменяет только root finalizer под одним contract lease.
...
- Review: exact balanced review produced one actionable BA0 replay finding, but its mandatory creation-bound exit evidence was missing at the hard deadline, so the result remained diagnostic-only and no replacement reviewer was created. Root reproduced the finding, completed TDD remediation, reran focused/package/full validation, and closed the gate only after the user's explicit 2026-07-25 root self-review override. Review mode is `self-review-limited`; independence is not claimed.
- Version: validated implementation/docs candidate `2.4.0-alpha.9`; no standalone prerelease commit was created, and the authorized M8 release folds this candidate into the stable `2.4.0` commit.
- Commit: included in the staged M8 stable `2.4.0` release candidate; the complete 15-file index passed the commit-gate against `v2.4.0-alpha.8`, and commit creation remains the next M8 action.

### M8. Stable GitHub release и remote verification

- Status: In progress — stable `2.4.0` candidate staged; commit-gate passed; candidate commit/publication pending
...
- Review: M7 root self-review override applies only to the exact failed review transport; M8 still requires the immutable candidate, clean install, pre-publication exact-version audit and post-publication evidence below.
- Version: `2.4.0`, the selected stable minor target from authoritative `v2.4.0-alpha.8`.
...
| A-103 | yes | discovery / R-032 M7 continuation | `gpt-5.3-codex-spark` | low | `turn.failed`, no valid result | Exact discovery failed without eligible model-specific fallback evidence; targeted root recovery used only the already-known M7 specification, README and owner/test paths | exact run `20260725T091833Z-b5d7b530ba`: nonzero exit, no valid result, no fallback |
| A-104 | yes | implementation / R-032 M7 runtime contract completion | `gpt-5.6-terra` | medium | completed; handoff abandoned, root completion authorized | Completed the runner/recovery M7 contract and focused regression. Exact pre-existing dirty overlap selected terminal-abandonment-v5; root independently attributed the allowed diff and recorded same-scope completion without accepting a handoff | exact run `20260725T092607Z-1a3badd425`: observed model/agent, `turn.completed`, exit `0`, process tree stopped, valid result, root completion |
| A-105 | yes | implementation / R-032 M7 package transition registry | `gpt-5.6-terra` | medium | completed; handoff abandoned, root completion authorized | Added static exact O-class transition derivation and positive/negative validator coverage. Generated ignored bytecode caused outside-set drift and terminal-abandonment-v1; root removed only those artifacts and independently accepted the two-file result | exact run `20260725T094850Z-00ebf3d42b`: observed model/agent, `turn.completed`, exit `0`, process tree stopped, valid result, root completion |
| A-106 | yes | review / R-032 M7 progressive review | `gpt-5.6-terra` | medium | exact transport failed; diagnostic `REVISE` only | The read-only semantic result found one high BA0 no-replace replay defect, but the process lifecycle lacked mandatory creation-bound exit evidence at the hard deadline. Root cancelled it, created no replacement reviewer, reproduced the fault and kept the gate blocked until the explicit user override | exact run `20260725T102005Z-a34b93a9ad`: `turn.completed`, valid result, missing exit evidence, cancelled, process tree stopped, result not accepted |
| A-107 | yes | implementation / R-032 M7 BA0 replay remediation | `gpt-5.6-terra` | medium | completed; handoff abandoned, root completion authorized | Made existing BA0 receipt/intent/visible/handoff replay exact for clean and incident outcomes, deterministic across restarts and fail-closed on mismatch/tamper. Pre-existing overlap plus two generated bytecode files selected terminal-abandonment-v3; root attributed the allowed diff and reran all focused checks | exact run `20260725T103704Z-39f3fafd0c`: observed model/agent, `turn.completed`, exit `0`, process tree stopped, valid result, root completion |
...
### 2026-07-25 — M7 legacy migration, documentation and package closure

- Writer lifecycle: A-104, A-105 and A-107 each completed through the exact Terra/medium implementation route. Existing task dirtiness and generated ignored bytecode prevented formal handoff acceptance, so owner-derived terminal-abandonment-v5/v1/v3 restored exact vacancy. Root removed only generated artifacts, independently attributed each allowed file set, reran focused checks and recorded the original-request same-scope root-completion audit; no replacement or recovery writer was created.
- Changed: the runner/recovery bridge now supports the 2.4.0 reader floor, legacy preservation and terminal migration contract; `project_migration.py` owns automatic first-Build I0 setup, permanent key/capability and immutable BA0 anchor state, clean B0 versus breach BS, exact C/E/O aliases, guarded mutation/observation contexts, protected work, retirement and compaction. Package validation statically derives exact registered ordinary-transition IDs without weakening fixed-model detection.
- Replay remediation: diagnostic review A-106 exposed that a crash after `bootstrap-receipt.json` made the next identical BA0 call fail at no-replace. Root reproduced the exact red. A-107 now reload-validates and replays immutable receipt, intent, visible outcome and handoff for both clean and incident branches; mismatched attempt/evidence/verdict, mixed outcome state and tampered artifacts fail closed.
- Documentation: README EN/RU, SKILL, CONTRIBUTING, manifest and changelog describe one model — “parallel tasks, one writer per lane, one integrator” — plus automatic setup/continue, `setup-required`, permanent key/anchor lock, BA0 replay/compaction, clean B0/breach BS, protected legacy work, conditional mixed-version limitation, separate `O6` commit and `O7` publication fences, troubleshooting and rollback.
- Validation: focused migration suite passes `17` tests with one Windows symlink-permission skip. Final repository-wide discovery passes `578` tests with nine platform skips in `504.879s`; package validation and `git diff --check` are green. The complete staged 15-file stable candidate passes `python scripts/validate_package.py --commit-gate` for the exact `2.4.0-alpha.8 -> 2.4.0` transition. Two earlier full attempts each hit a different existing Windows timing/filesystem transient; both exact tests passed in isolation and inside the final green full run, without production relaxation.
- Review override: A-106 produced a semantically useful `REVISE` but lacked mandatory creation-bound exit evidence, so it remained diagnostic and no replacement reviewer was created. After the single finding was remediated and all root checks were green, the user explicitly authorized `self-review-limited` closure on 2026-07-25. Root acceptance coverage is AC-19/20/21/23/24/28/30/35/36/38 `MET`; no remaining actionable finding was found, and independence is not claimed.
- Minimality: root-only same-scope closure reuses Python stdlib, existing owner-private durability, recovery registry and package validator; no dependency, service, provider, hosted coordinator or compatibility fallback was added.
- Version record: source `plugins/openbuild/.codex-plugin/plugin.json`; impact `minor`; validated implementation/docs candidate `2.4.0-alpha.9` is not committed separately and is folded into the separately authorized stable `2.4.0` M8 release commit. Stable tag, GitHub Release and remote install/smoke remain external M8 evidence.
